...
两种目标来源，对应 state 到底说不说得清：
  ...
  - `hard` —— notes 段点名了目标对象（`取消` + `订阅`），答案唯一，one-hot。
    这类样本同样参与 ECE、观测 Brier 和 NLL；硬标签可用于训练和评估校准。
    provenance 分组用于说明目标来源，不代表硬标签不具备校准信息。
...
时，目标为有效集上的均匀分布，而不是随意选择一个高置信答案。
分来源报告便于说明目标的构造；hard 与 soft_targets 的混合总体也可以合法评测。
