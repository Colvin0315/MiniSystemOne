...
一键离线流程见 [scripts/quickstart.py](scripts/quickstart.py)；自定义任务的完整教程见 [完成第一个任务](docs/FIRST_TASK.md)。
...
产物是 `tokenizer.json` 和 `tokenizer_config.json`。`train_tokenizer.py` 与 `train_mlm.py` 的纯合成训练都必须使用 `--synthetic_only`；空 `--pretrain_path=`、`--en_path=` 不再表示显式禁用来源。使用外部文本时去掉 `--synthetic_only`，提供有效的本地语料路径；路径不存在会报错，不会自动退回纯合成训练。`--skip_eval` 仅在这个小规模演示中跳过压缩率检查。
...
Python 有两个并列入口：[model/inference.py](model/inference.py) 提供上述单问题 `DecisionPredictor`；根目录 [inference.py](inference.py) 提供类型化接口。
...
训练 JSONL 的 `candidates` 是带元数据的对象，推理 JSON 的 `candidates` 则是字符串列表。二者用途不同。决策数据每行必须有与所在文件匹配的 `split`（例如 `train.jsonl` 中为 `"split": "train"`）；缺失或不匹配会报错。
...
保持同一训练代码版本、原始数据、词表、训练设置与总 `epochs` 不变。`--max_steps N` 是固定的 optimizer 总步数预算及学习率 horizon，恢复时必须保持不变，不能删除或增大。临时暂停用 `--stop_after_steps`，恢复时可去掉此暂停参数，但不改变总预算；快速路线的五步训练已经完成五步预算，不是等待恢复的更长训练。MLM 同样支持 `--resume`。`--resume` 与非空 `--encoder` / `--init_checkpoint` 互斥，恢复命令须删除初始化参数。旧版本 checkpoint 只用于权重初始化，并新建优化器和训练计划，不能精确续训；缺失或不兼容的续训状态会被拒绝。
...
**一个容易混淆的点：**硬标签交叉熵和 Brier 也能学习条件概率。软目标让本项目更容易直接检查分布误差，但不保证模型天然校准。评测用 `distribution_l2` 报告平方分布距离，`expected_brier` 则逐样本加上 $1-\sum_i t_i^2$ 后取均值；one-hot 目标下二者均等于观测标签 Brier。训练参数和内部 loss logger 保留 `brier` 名称，评测没有旧 `brier` 别名。更多解释见 [校准文档](docs/CALIBRATION.md)。
...
| `distribution_l2` | 预测分布离目标分布多远？ | 有效候选上的平方差求和，再跨样本平均 |
| `expected_brier` | 给定目标分布下的观测标签 Brier 期望是多少？ | `distribution_l2` 加上 $1-\sum_i t_i^2$ 的均值；one-hot 时二者相同 |
| `ece` | 置信度桶内的预测与正确率是否一致？ | 越小越好，但单独看它不够 |
| `ordinal_mae` | 预测等级分布移动多远才能到目标？ | 等级单位的 Wasserstein-1 距离 |
| `expected_score_mae` | 期望分误差有多大？ | Score 专用 |

按 `source` 和 `provenance` 分组读结果，再看汇总。当前输出包含 `all`、各 provenance 分组及四种非 `hard` 来源的 `soft_targets` 聚合（与 `all` 相同时省略），不表示硬标签无法评测校准。旧 `brier` 指标和 `calibration` 聚合均无兼容别名；历史图片可能仍使用旧名。
...
效率报告与任务效果分开阅读。候选数、state 长度、batch、分词和打包开销都会改变端到端延迟。架构测试覆盖候选重排、分块和前缀隔离；回归测试覆盖序数、截断、弃权、推理和续训。GPU 续训一致性测试见 [详细入门指南](docs/QUICKSTART.md)：`test_resume.py` 有 CUDA 时自动运行，`test_training_integration.py` 还需设置 `MINISYSTEMONE_GPU_TESTS=1`。
