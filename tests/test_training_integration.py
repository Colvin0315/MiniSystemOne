    ...
    def test_both_trainers_resume_to_same_weights(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            for split in ('train', 'val'):
                (root / (split + '.jsonl')).write_text('\n'.join((json.dumps(dict(r, split=split)) for r in records)), encoding='utf-8')
            ...
            for stage, extra in [('mlm', ['--pretrain_path', str(corpus), '--en_path', str(corpus), '--en_share', '0', '--n_docs', '10', '--n_synth', '0']),
