"""Behavior regressions; model-forward checks require CUDA."""
import unittest
import json
import tempfile
...
class CorrectnessTests(unittest.TestCase):
    ...
    def test_fractional_levels_survive_data_collation_and_collection(self):
        ...
        with tempfile.TemporaryDirectory() as folder:
            ...
            model = MiniSystemOneForDecision(DecisionConfig(hidden_size=32, num_hidden_layers=1, vocab_size=128)).cuda()
            col = collect(model, ds, 'cuda', batch_size=1)
    ...
    def test_abstention_is_not_covered_and_soft_targets_are_used(self):
        ...
        np.testing.assert_allclose(coverage, [0., 0., .5])
        np.testing.assert_allclose(risk, [np.nan, np.nan, .25])
        np.testing.assert_allclose(threshold, [np.inf, .99, .8])
        coverage, risk, threshold = risk_coverage_curve(p[:1], t[:1], abstain_idx=0)
        np.testing.assert_allclose(coverage, [0., 0.])
        self.assertTrue(np.isnan(risk).all())
        np.testing.assert_allclose(threshold, [np.inf, .99])
    ...
    def test_equal_confidence_thresholds_group_ties(self):
        ...
        np.testing.assert_allclose(risk, [np.nan, .5])
        np.testing.assert_allclose(thr, [np.inf, .8])
    ...
    def test_forward_honors_brier_normalize(self):
        model = MiniSystemOneForDecision(DecisionConfig(hidden_size=32, num_hidden_layers=1,
            num_attention_heads=4, num_key_value_heads=2, vocab_size=32)).cuda().eval()
