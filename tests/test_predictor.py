...
class InferenceTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.predictor = DecisionPredictor(cls.ckpt, str(ROOT / 'model'), device='cuda')
    ...
    def test_calibration_must_match_checkpoint(self):
        ...
        with self.assertRaises(ValueError):
            DecisionPredictor(self.ckpt, str(ROOT / 'model'), device='cuda', calibration=str(path))
    ...
    def test_checkpoint_without_tokenizer_fingerprint_is_rejected(self):
        ...
        with self.assertRaisesRegex(ValueError, 'tokenizer_sha1'):
            DecisionPredictor(str(path), str(ROOT / 'model'), device='cuda')
